...
import org.apache.commons.lang3.ArrayUtils;import org.geoserver.config.GeoServerDataDirectory;import org.geoserver.config.util.SecureXStream;
import org.geoserver.platform.GeoServerExtensions;import org.geoserver.platform.resource.Resource;import org.geotools.util.logging.Logging;
import java.io.InputStream;import java.io.OutputStream;import java.util.ArrayList;import java.util.List;import java.util.UUID;import java.util.logging.Level;import java.util.logging.Logger;import java.util.stream.Collectors;
public class LinkingManglerRuleDAO {
    ...
    public static final String LINKING_MANGLE_RULES_PATH = "web-linking-mangling/linking-mangler-rules.xml";
    public static final String LINKING_MANGLE_RULES_PATH_TMP = "web-linking-mangling/%s-linking-mangler-rules.xml";
    ...
    static {
        ...
        xStream.allowTypes(
                new Class[] {LinkingManglerRule.class, LinkingManglerRuleDAO.LinkingManglerRuleList.class});
    }
        ...
        if (resource.getType() == Resource.Type.RESOURCE) {
            try (InputStream inputStream = resource.in()) {
                if (inputStream.available() == 0) {
                    ...
                    LinkingManglerRuleList list = (LinkingManglerRuleList) xStream.fromXML(inputStream);
                    return list.rules == null ? new ArrayList<>() : list.rules;
                }
            } catch (Exception exception) {
                throw new LinkingManglerException(exception, "Error parsing linking mangler rule files.");
            }
        } else {
    ...
    public static void saveOrUpdateLinkingManglerRule(LinkingManglerRule linkingManglerRule) {
        ...
        Resource tmplinkingManglerRules = getDataDirectory().get(getTmpLinkingManglerRulesPathPath());
        saveOrUpdateLinkingManglerRule(linkingManglerRule, linkingManglerRules, tmplinkingManglerRules);
    ...
    private static void writeLinkingManglerRules(List<LinkingManglerRule> linkingManglerRules, Resource output) {
